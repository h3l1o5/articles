...
`agent-skills` 這個 repo 想做的，就是把那一套「不能跳」的紀律外顯成 agent 看得懂的流程文件——21 個 skill，覆蓋從 idea refinement 一路到 shipping 與 launch 的整個生命週期。它不是另一套 prompt 模板，也不是「給我寫個 React component」那種工具型 plugin，而是一份**寫給 AI 的工程紀律手冊**。

而且很值得一提的是：它**完全模型無關**。docs 目錄裡同時有 Claude Code、Cursor、Gemini CLI、Windsurf、OpenCode、GitHub Copilot、OpenAI Codex、Kiro IDE 的安裝指南——八個平台。作者刻意把它做成跨廠商的中性規格。這在 2026 年 plugin 生態系開始百家爭鳴的時刻，是一個很有遠見的決定。

repo 在 2026 年 2 月開張，到 4 月中已經 13k stars、21 個 skill，作者每週都在更新，社群貢獻也很活躍。MIT 授權。

> **2026-04-12 更新：** 本文首發時 repo 有 9k stars 跟 19 個 skill。四天後已成長到 13k stars、21 個 skill（新增 `source-driven-development`），平台支援從 7 個擴展到 8 個（新增 Kiro IDE）。同時也修正了原文中幾個在深入研究 gstack 跟 superpowers 之後發現需要校正的觀點——特別是 Common Rationalizations 的「原創性」問題，以及新增的三方生態定位比較。

接下來我會帶你拆解：在這個 repo 的語境裡，「skill」到底是什麼東西、21 個 skill 各做什麼、6 個我認為最值得學習的設計、整個 repo 的招牌設計模式，最後是我對它的誠實評論——包含哪些做得漂亮、哪些其實有點粗糙。
...
## 21 個 skill 全圖：六階段開發生命週期

21 個 skill 並不是隨意放在 `skills/` 下，作者按照軟體開發的六個階段做了清楚的分組。我把全部列出來，每個一句話：
...
- **`source-driven-development`** — *（4/9 新增）* 每個 framework-specific 決策都必須有官方文件支撐。DETECT → FETCH → IMPLEMENT → CITE 流程，防止 agent 從過時的 training data 寫出已棄用的 API 用法
...
- **`performance-optimization`** — 測量先行、Core Web Vitals、bundle analysis。*（4/11 更新：新增 font subsetting/variable fonts section 跟 `yieldToMain` pattern）*
...
新增的 `source-driven-development` 值得特別說一下。它解決的問題非常具體：agent 從 training data 寫出過時的 API 用法——例如用已棄用的 React lifecycle method、錯誤的 Next.js config syntax、過時的 Tailwind class。這在 2026 年的 AI coding 場景裡是每個人都踩過的坑。解法是 DETECT → FETCH → IMPLEMENT → CITE 四步流程，加上明確的 source hierarchy：官方文件 > 技術 blog > MDN > caniuse。每個 framework-specific 寫法都要附 citation。這個 skill 是社群貢獻（Federico Bartoli），說明 repo 的 contribution pipeline 是真的有人在用。
...
讀完 21 個 skill 之後你會發現幾個重複出現的設計手法。這些是 addyosmani 的「個人風格」，也是這個 repo 跟其他 skill collection 最不一樣的地方。
...
addyosmani 為這個 repo 提供了八種不同 agent 的安裝方式，這是它跟絕大多數 Claude-only skill repo 最大的差異。
...
任何 repo 我都會給一份誠實的好壞評。13k stars 不代表沒有問題。

### 做得漂亮的地方

1. **Common Rationalizations 是這個 repo 做得最系統化的設計。** 每個 skill 都必須包含 rationalization 表格——這個「覆蓋率」是其他 repo 沒做到的。公平地說，這個 pattern 並非 addyosmani 原創：obra/superpowers 在 2025 年 10 月就有了 Red Flags table 跟 Rationalization table，比 agent-skills（2026 年 2 月）更早。但 superpowers 的 rationalization 只出現在少數 rigid skills（TDD、debugging、verification），而 agent-skills 把它變成**每個 skill 的必備章節**，並在 `docs/skill-anatomy.md` 裡寫成規範。這種「全面覆蓋」的做法讓它在防止 agent 偷懶上更一致。
...
1. **幾乎沒有自動化測試。** 整個 repo 的 CLAUDE.md 寫著 `npm test: Not applicable (this is a documentation project)`。所有貢獻都是人眼審閱，沒有 skill 品質的客觀驗證機制。對一個有 13k stars 的 repo 來說這有點可惜。好消息是社群已經注意到這個問題——Issue #51 提出了 evaluation framework，Issue #60 提出了 CI skill validator——但目前尚未落地。對比之下，gstack 已經有三層測試體系（static validation + E2E via `claude -p` + LLM-as-judge）。

2. **沒有效能評量數據。** repo 從頭到尾沒有任何「採用這些 skill 後 agent 輸出品質提升 X%」的實測數據，全部是 "trust me, this works" 的哲學論述。HN 上的討論也指出這一點——缺乏 A/B testing 或量化 benchmark。對一個目標是「production-grade」的 repo 來說，這是一個明顯的空白。

3. **56% non-invocation rate 是一個真實問題。** HN 討論中有人指出 agent 有超過一半的時間不會自動調用 skills——即使 description 寫得符合規範。社群的共識是：靠 description-based dispatch 做自動觸發並不可靠，主動用 slash command（`/build`、`/review`、`/ship`）呼叫才是可靠路徑。這意味著 repo 裡精心設計的 description 觸發條件，在實際使用中的效果可能遠低於預期。

4. **`idea-refine` 是個明顯的異物。** 它的 frontmatter 不是第三人稱動詞開頭、結構更像 Anthropic 官方 skill 而非 repo 自家規範，而且是唯一一個有 `scripts/` 子目錄的 skill。看起來是從別處移植進來但沒重寫。

5. **部分 skill 哲學重複但沒互相 link。** 例如 `security-and-hardening` 的 Three-Tier Boundary 跟 `spec-driven-development` 的 Boundaries 章節概念高度重疊但沒互相 reference。違反了它自己 `docs/skill-anatomy.md` L406-415 的規範。

6. **Anti-rationalization 的 context 成本不可忽視。** 每個 skill 都載入 rationalization 表格、flow chart、verification checklist，如果一次啟動多個 skill，token 壓力明顯。作者在 `docs/getting-started.md` 自己也承認這點，建議「不要一次全部載入」。但這個自我承認其實暴露了一個更深的問題：當你的紀律變成負擔，使用者就會繞過它。

7. **覆蓋面偏 web/frontend。** 21 個 skill 裡幾乎沒有 backend service、data engineering、ML/LLM-ops 的內容。對一個 Google Chrome 工程主管寫的 repo 來說這不意外，但如果你做的是後端或數據工程，你會發現很多概念可以借鑑但沒辦法直接套用。

### 在 2026 年 skill pack 生態系裡的定位

寫這篇文章的時候，我只拿 agent-skills 跟 Anthropic 官方 skills 比。現在深入研究過 gstack（garrytan/gstack，69k stars）跟 superpowers（obra/superpowers，147k stars）之後，可以給一個更完整的定位。

**三者約束的維度完全不同：**

| Skill Pack | 約束什麼 | 核心機制 |
|-----------|---------|---------|
| **agent-skills** | **Knowledge**——確保 agent 遵循 best practices | 靜態 Markdown 文件，跨 8 個平台 |
| **superpowers** | **Process**——確保 agent 走完整流程不跳步驟 | SessionStart hook 強制注入，Iron Law 閘門 |
| **gstack** | **Perspective**——讓 agent 從不同角色的視角審視 | 23 個角色型 slash command，內建 browser daemon |

**agent-skills 是三者中最「可攜」也最「輕量」的。** 純 Markdown，零 build step，零 runtime dependency，支援 Claude Code、Cursor、Gemini CLI、Windsurf、OpenCode、Copilot、Codex、Kiro IDE 八個平台。superpowers 主要綁 Claude Code（雖然有 Cursor 跟 Gemini 支援），gstack 也主要綁 Claude Code。

**但 agent-skills 也是最「被動」的。** superpowers 有 SessionStart hook 在每個 session 開始時強制注入紀律框架；gstack 有 proactive routing 根據對話內容自動觸發 skill。agent-skills 的自動觸發依賴 description-based dispatch，而 HN 討論揭露這個機制的 non-invocation rate 高達 56%——意思是 agent 有超過一半的時間不會自動調用 skills。相比之下，superpowers 的 1% rule 幾乎是強迫式的（只要有 1% 機率適用就必須調用），gstack 的 routing rules 寫進 CLAUDE.md 也相當有效。

**三者的互補方式：**

- **agent-skills** 提供跨平台的工程知識基底——Three-Tier Boundary、Common Rationalizations、Prove-It Pattern、Chesterton's Fence 這些概念不管你用什麼 agent 都適用
- **superpowers** 提供最嚴格的流程約束——TDD Iron Law、verification gate function、「Delete means delete」的紀律是 gstack 跟 agent-skills 都沒有的
- **gstack** 提供最完整的工具鏈——browser daemon、cross-model review、design pipeline、shipping automation 是另外兩個 repo 都沒有的

如果你只用一個 agent（例如只用 Claude Code），superpowers 或 gstack 的 enforcement 機制比 agent-skills 的被動 Markdown 更有效。如果你用多個 agent（Claude + Cursor + Codex + Gemini），agent-skills 是唯一一個能在所有平台上一致運作的選擇。

`code-simplification/SKILL.md` 第 3 行那句直接致敬 Anthropic `code-simplifier` plugin 的話，其實是這個 repo 對自己定位最清楚的宣告：**Anthropic 是原廠工具，addyosmani 是跨廠商的紀律規範。** 在加入 gstack 跟 superpowers 的脈絡後，更精確的說法是：**agent-skills 是知識層，superpowers 是流程層，gstack 是工具層——三者在不同軸上，可以疊加使用。**
...
- 你的工作領域是 backend / data / ML，目前 21 個 skill 對你的覆蓋率不高
- 你的 AI usage 是以「對話 + 修改」為主而非「agent 自主執行」為主——這個 repo 的價值在 agent 自主執行越長的場景越顯著
- 你只用 Claude Code 而且已經裝了 superpowers 或 gstack——它們的 enforcement 機制更強，agent-skills 的被動 Markdown 在這個場景下價值較低
...
把資深工程師的本能變成 AI agent 的紀律——這件事看起來很簡單，但實際上沒幾個人做得到 addyosmani 這個程度的具體與徹底。在 superpowers（147k stars）跟 gstack（69k stars）的光環下，agent-skills 的 13k stars 可能顯得不起眼。但它有一個那兩者都沒有的優勢：**它是唯一一個真正跨平台的 skill collection。** 如果你的團隊裡有人用 Claude、有人用 Cursor、有人用 Gemini、有人用 Codex——agent-skills 是唯一能讓所有人共享同一份工程紀律的選擇。光是這一點，它在生態系裡的位置就不可替代。
